...
public class IssueService {
    ...
    private boolean userOwnsIssue(User user, Issue issue) {
        return issue.getCreatedBy() != null &&
    ...
    public void deleteIssue(UUID issueId, User user) {
        ...
        if (!userOwnsIssue(user, issue)) {
            throw new AccessDeniedException("You can't delete this issue");
        }

        issueRepository.delete(issue);
    }

    public Page<Issue> getIssuesByProject(UUID projectId, boolean mine, User user, Pageable pageable) {
        if (mine) {
